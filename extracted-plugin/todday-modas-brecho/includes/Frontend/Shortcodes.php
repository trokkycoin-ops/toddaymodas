...
class Shortcodes {
    public static function register(): void {
        ...
        // Aliases legados (páginas existentes usam tm_*)
        add_shortcode('tm_home', [self::class, 'render_vitrine']);
        add_shortcode('tm_carrinho', [self::class, 'render_cart']);
        add_shortcode('tm_checkout', [self::class, 'render_checkout']);
        add_shortcode('tm_whatsapp', [self::class, 'render_whatsapp_btn']);
        add_shortcode('tm_contato', [self::class, 'render_whatsapp_btn']);
    }

    public static function render_vitrine(array $atts = []): string {
        ob_start();
        
        // Forçar atualização de versão para limpar cache
        header('Cache-Control: no-cache, no-store, must-revalidate');
        header('Pragma: no-cache');
        header('Expires: 0');
        
        // Usar template marketplace por padrão (estilo Mercado Livre/Shopee)
        $template_path = TODDAY_MODAS_PATH . 'templates/vitrine-marketplace.php';
