...
class ImportExportService {
    /**
     * Previne injeção de fórmula em planilhas: células que começam com
     * =, +, -, @ (ou tab/CR) ganham um apóstrofo de escape.
     */
    ...
    public static function export_products_csv(): void {
        ...
        foreach ($products as $p) {
            ...
            fputcsv($out, [
                $p->get_id(),
                self::csv_safe($p->get_name()),
                self::csv_safe($p->get_sku()),
                $p->get_regular_price(),
                $p->get_sale_price(),
                $p->get_stock_quantity() ?? '',
                self::csv_safe($categories),
                self::csv_safe($condition),
            ]);
        }
    ...
    public static function export_orders_csv(): void {
        ...
        foreach ($orders as $order) {
            fputcsv($out, [
                ...
                $order->get_status(),
                self::csv_safe($order->get_formatted_billing_full_name()),
                self::csv_safe($order->get_billing_email()),
                $order->get_total(),
                $order->get_shipping_total(),
                self::csv_safe($order->get_payment_method_title()),
            ]);
        }
