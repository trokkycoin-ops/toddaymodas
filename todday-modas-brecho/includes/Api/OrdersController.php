...
class OrdersController {
    ...
    public static function get_order(WP_REST_Request $request): WP_REST_Response {
        ...
        if (!self::user_can_view_order($order, $user_id)) {
    ...
    private static function user_can_view_order(array $order, int $user_id): bool {
        if (CapabilityMatrix::can_manage_store()) {
            return true;
        }
        ...
        if ($user && in_array('todday_vendedor', $user->roles, true) && (int) $order['vendor_id'] === $user_id) {
            return true;
        }
        return false;
    }
