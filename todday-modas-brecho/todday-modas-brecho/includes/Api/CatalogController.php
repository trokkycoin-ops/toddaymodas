...
class CatalogController {
    ...
    public static function format_product($product, bool $full_details = false): array {
        ...
        $main_image = $image_id ? wp_get_attachment_url($image_id) : (get_post_meta($id, '_todday_image_url', true) ?: '');
        ...
        $available_sizes = json_decode((string) get_post_meta($id, '_todday_available_sizes', true), true);
        $colors = json_decode((string) get_post_meta($id, '_todday_colors', true), true);
        $measurements = json_decode((string) get_post_meta($id, '_todday_measurements', true), true);
        ...
        $data = [
            ...
            'condition' => $condition,
            'size' => $size,
            'available_sizes' => is_array($available_sizes) && $available_sizes ? $available_sizes : [$size],
            'available_colors' => is_array($colors) ? $colors : [],
            'brand' => get_post_meta($id, '_todday_brand', true) ?: 'Todday Modas Brechó',
            'fabric' => get_post_meta($id, '_todday_fabric', true) ?: '',
